...
pipe(
  ...
  (tl) => tl.addLabel(l2),
  line('#top'),
  (tl) => tl.from('#main-page-descr', { opacity: 0, x: 40 }, '-=0.2'),
  btn('#top', '-=0.2'),
  scroller('#top', '-=0.3', 0.1),
  (tl) => tl.from('#iphone-app img', { opacity: 0 }, l2 + '-=0.2'),
  (tl) => tl.from('#incubator img', { opacity: 0, x: -100 }, l2 + '-=0.2'),
);